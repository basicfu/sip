    ...
    fun list(vo: DictVo): PageInfo<DictDto> {
        val page= selectPage<DictDto>(
            example<Dict> {
                ...
                orderByAsc(Dict::sort)
            }
        )
        val parentList = page.list
        val ids = page.list.map { it.id!! }
        if(ids.isNotEmpty()){
            val list =
                to<DictDto>(mapper.selectBySql("SELECT d1.id,d1.name,d1.value,d1.lft,d1.rgt,d1.lvl,d1.fixed,d1.sort FROM dict AS d1,dict AS d2 " +
                        "WHERE d1.lft > d2.lft AND d1.lft<d2.rgt AND d2.id in (${StringUtils.join(ids,",")}) and d1.isdel=0"))
            parentList.forEach {
                chidren(it, list)
            }
            page.list=parentList
        }
        return page
    }
